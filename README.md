...
$ npm init dominant-app my-app
```

`npm` will automatically install [create-dominant-app](https://github.com/guiprav/create-dominant-app) and run it for you. `create-dominant-app` will create your new app directory, initialize `package.json` in it, and install all the necessary dependencies.

Once the script finishes, you should be able to change into the newly created directory and start the Dev Server:
...
The Dev Server will automatically open http://localhost:8080 on your browser, but if for some reason it doesn't, you can manually open that URL yourself.

You can make changes to `public/index.html` and `src/index.tsx` and work on your own app from there. The Dev Server will automatically refresh your app whenever you save changes to the source code.
...
When a component function or Component class are supplied, any  **children** supplied are stored in `props.children` before **props** is forwarded to the component function or constructor.
...
The `switch` analog to `d.if`.

```jsx
let keys = ['harder', 'better', 'faster', 'stronger'];
let i = 0;

document.body.append(d.switch(() => keys[i], {
  harder: <div>HARDER</div>,
  better: <div>BETTER</div>,
  faster: <div>FASTER</div>,
  stronger: <div>STRONGER</div>,
}));
